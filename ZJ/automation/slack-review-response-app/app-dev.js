...
async function sendDataToRetool(reviewID, response) {
  ...
  try {
    const res = await fetch('http://retool:8080/retool/v1/workflows/ce1340cc-449c-4f2e-9514-8056432d866a/startTrigger', {
      method: 'POST',
      body: JSON.stringify(payload),
      headers: {
        'Content-Type': 'application/json',
        'X-Workflow-Api-Key': `${process.env.DEV_RETOOL_API_KEY}`
      }
    });
...
app.receiver.client.on('disconnect', (error) => {
  console.error("Disconnected from Slack:", error);
});
